...
//This file use with livesearchthai.php
  ...
  $fields = array("status_desc","s_fname","degree_name","major_name","ins_name","dep_name","location_name","plant_name","c_name","host_name","country_name","application_dateS");
  $conditions = array();
...
LEFT JOIN institute l ON l.ins_id = d.ins_id
...
// if there are conditions defined
    if(count($conditions) > 0) {
        ...
        $query .= " WHERE " . implode (' AND ', $conditions ) . " AND a.origin_id ='1'"; // you can change to 'OR', but I suggest to apply the filters cumulative
        //echo "<br>".$query;
    }else {
      $query .=" WHERE a.origin_id ='1'";
     }
...
<head>
        ...
        <title>Report Management : Participant (Non-Thai)</title>
            ...
            <!-- Header -->
                <header class="w3-container w3-theme w3-padding w3-round"  id="myHeader">
                    <div class="w3-center">
                        ...
                        <h3 class="w3-xxlarge w3-animate-right">Participant (Non-Thai)</h3>
                    </div>
                </header>
            </div>
         </div>
...
<div class="field" id="searchform" >
 ...
 <select id="plant" name="plant_name">
   ...
   <option value="PRB">PRB</option>
      ...
      <select id="channel" name="c_name">
         <option value="">Select..</option>
        <option value="EXCHANGE">EXCHANGE</option>
        <option value="IEASTE">IEASTE</option>
        <option value="WACE">WACE</option>

      </select>
</div>        <div class="field" id="searchform" >
      <label for="host">Host University</label>
      <input type="text" id="host" name="host_name" placeholder="Host..">
</div>
        <div class="field" id="searchform" >
      <label for="country">Country</label>
      <input type="text" id="country" name="country_name" placeholder="Country..">
        </div>
              <div class="field" id="searchform">
...
?>
        ...
        <td class="text-left"><?php echo $row['c_name'];?></td>
        <td class="text-left"><?php echo $row['host_name'];?></td>
          <td class="text-left"><?php echo $row['country_name'];?></td>
