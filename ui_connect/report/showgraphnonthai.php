...
//This file use with livesearchthai.php
  ...
  $fields = array("plant_name","location_name","status_desc","degree_name","dep_name","major_name","ins_name","host_name","c_name","country_name", "application_dateS");
  $conditions = array();
  ...
  //To check does file is sent to this form by method post yet?
    ...
    $homeuniver   = $_POST['ins_name'];
    $hostuniver   = $_POST['host_name'];
    $channel      = $POST['c_name'];
    ...
    switch ($type) {
      ...
      case "major_name":
        $headname = "Major";
          break;
      case "ins_name":
        $headname = "Home University";
          break;
      case "host_name":
        $headname = "Host University";
          break;
      case "c_name":
        $headname = "Channel";
        break;
...
// builds the query all table that we're looking for
  $query = "SELECT plant_info.plant_name,
            ...
            INNER JOIN host_university ON host_university.host_id = non_thai.host_id
...
// if there are conditions defined
    if(count($conditions) > 0) {
        ...
        //to trap if does type is filled or not? if yes add GROUP BY condition
        if ($_POST['type'] != ''){
            $query .= " GROUP BY ".$type;
        }
        //echo "<br>".$query; //to check query command
    }
...
//fetch number of data and data type which are selected to show
 ...
 if (($_POST['type'] != '') and ($s_num != '' )){ ?>
                     ...
                     <div class="container">
                       <div class="row">
                         <div class="col-xs-12 big-logo-container">
                           <h1 class="big-logo">Non-Thai Students Searching</h1>
                         </div><!--/.col-xs-12 -->
                       </div><!--/.row -->
                     </div><!--/.container -->
                   </div><!--/.container-fluid -->
      ...
      <?php if (($headname == "Department") or ($headname == "Home University") or ($headname == "Major") or ($headname == "Country")){ ?>
                  <br>
